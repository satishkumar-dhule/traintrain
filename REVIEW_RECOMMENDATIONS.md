...
**Method:** A 10-agent specialist review team (backend architecture, data integrations, AI/insights, Svelte frontend, SPA frontend, security, testing/devops, observability/SRE, performance/resilience, docs & data). Findings were cross-checked with direct source inspection.

---

## 1. Executive summary

The application is **functionally strong**: a coherent vertical-slice architecture (`src/slices/`), a clean separation of `core/` infrastructure, honest `data_source` reporting, a genuinely good hermetic mock-based test suite, correct security headers, and a real (not fabricated) metrics stack. **The core risks are drift and dead code** — the code has evolved (Svelte rewrite, `fanout_n2` refactor, ~10 more slices and sources, an SRE/resilience layer) faster than the docs, the quality gates, and the cleanup of superseded code paths.

**Top 5 things to fix first (biggest correctness/integrity impact):**

1. **Fabricated live data is still served as real** (ConfirmTkt/Ixigo/Erail/IndiaRailInfo/Etrain synthesize fake trains & waitlists that win the fan-out race and get labeled "live"). — *correctness / trust*
...
4. **Admin/observability endpoints are unauthenticated and internet-reachable**, and internal error strings/upstream URLs leak to clients; a spoofable `X-Forwarded-For` defeats rate limiting. — *security*
...
- `ixigo.rs`, `erail.rs`, `indiarailinfo.rs`, `etrain.rs` gate "success" on `html.contains("train")` / `html.contains("Train No")` and then emit `CT{src}{dst}` / `Erail {train}` / `IndiaRailInfo {train}` fake trains with fake times and at_src/at_dstn.
...
**Recommendation:** Move the proxy behind `config` (`RAILWAY_REPLIT_PROXY_BASE`) or drop the candidate. Reuse `state.http`. Delete the leftover clone. Fix the field mapping.
...
**Recommendation:** Remove the `.user_agent("railway-rs")` override (or drop UA from defaults — pick one source of truth); move per-source `Referer` to per-call headers; drop `Content-Type` from GET defaults.

### 2.4 Vanilla SPA / Tabs-registry is dead, and the live app is the Svelte build
**Files:** `static/index.html:14-15` loads only `theme-boot.js` + the built `assets/index-*.js`/`.css`; `static/tabs/` and `static/app.js` **do not exist**.

The whole `window.Tabs` registry pattern, `static/api.js`, `routes.js`, `ui.js`, `styles.css`, `palette.js` (≈125 KB) are **orphaned** — served by the backend (`src/web.rs:112` `ServeDir`) but never referenced by the served `index.html`. Skills `spa-tab`, `vertical-slice-rust` (Phase B), `observability-stack`, and `ui-layout-harness` describe this vanished architecture, misdirecting agents/contributors.

**Recommendation:** Decide the canonical UI (Svelte `frontend/` is clearly it), prune/relocate the orphaned `static/*` loose files, delete stale `assets/index-*.js` bundles (only keep the one referenced by `index.html`), and rewrite the affected skills to target `frontend/src/lib/pages/` + `lib/components/`.
...
`AppError::Internal(msg)` returns the full `msg` verbatim (HTTP 500), and `From<reqwest::Error>` embeds the **complete upstream URL**; `SourceUnavailable{reason}` echoes the reason. Confirmed at `error.rs:120-127`.

**Recommendation:** Return a generic `{"error":"internal server error"}` (optionally with a generated request-id); log detail server-side only (already done at `error.rs:117,121`). Keep user-facing `BadRequest`/`NotFound` messages.
...
`client_ip()` takes the **first** entry of the client-supplied `X-Forwarded-For` verbatim → a caller can rotate it per request to bypass the per-IP bucket completely. Default is 1000 rps / 1000 burst, and `.env.example` documents 100/50 (10× mismatch). All direct connections collapse into one `"unknown"` bucket.
...
- Client disconnect is not honored: the spawned tool loop uses `let _ = tx.send().await` everywhere and keeps running real rail tools + up to 4 model rounds after the client leaves.
- `/rail-api/ai/chat` is unauthenticated with no dedicated quota/cost cap (shares only the spoofable generic limiter).

**Recommendation:** Set `max_tokens`/`temperature`; `break` the loop when `tx.send` fails (receiver dropped); add a dedicated AI per-IP + global rate/cost cap; consider gating the endpoint on the configured `RAILWAY_AI_API_KEY`.
...
`Mutex<HashMap<String,Entry>>` has no capacity limit; `set_with_ttl` holds the global lock while `retain` scans the **entire map** on every write (O(n) under lock). High-cardinality keys (`live_status:{train}:{date}`, `availability:...:date:source`, `search:stations:{q}`) grow unbounded.
...
Re-serializes and copies the whole body (up to 64 MiB each) on every non-SSE response. Also means in-flight/byte metrics undercount AI/streaming traffic (`web.rs:89-98`), so load-shedding (`web.rs:260`) misses streaming load.
...
- No cache: fmt/clippy/test/release build is a cold rebuild every push; the Dockerfile invalidates the whole dependency build on any `src/` change.
...
**Recommendation:** Sync the tests to the real default (or vice-versa; the model was updated in commit 1136f6d without updating docs/tests).
...
Any markup change silently degrades to garbage rows. `pos_code` requires exact "Departed from NAME(CODE)" text.
...
`node --check` over `static/*.js` includes the minified **ESM** bundle → parse error (gate red). Vite `outDir: '../static'` with `emptyOutDir: false` → every build leaves stale hashed bundles (multiple `index-*.js` ≈868 KB each + `embed-*.js`), all served.
...
- **Live-station leftover**: `live_station/service.rs:119-121` computes `static_board(...)` then discards it.
...
### 4.3 Config drift & inconsist IE boolean-env semantics
- `RAILWAY_AI_ENABLED` uses inverse-falsy parsing (unknown→true); `ASKDISHA_ENABLED` uses strict truthy (unknown→false). Same knob type, two contracts.
...
### 4.5 Per-IP AI/observability endpoints used the spoofable limiter
...
- `static/ui.js:9,68,163,175-178` — raw `innerHTML` sinks; `esc()` isn't enforced; `statusCell` interpolates upstream text. CSP mitigates `<script>` but not event-handler/markup injection.
...
- Retry is fixed-delay (400 ms), retries 4xx/404, no jitter (`http.rs:55-81`); fan-out `RETRY_DELAY` fixed 200 ms (`fanout.rs:31`); POSTs have no retry at HTTP layer.
...
- AI SSE backpressure: a slow/disconnected client can block the upstream LLM read + bulkhead permit for the full round (`ai_chat/mod.rs:92`); no overall streaming deadline (`TOOL_TIMEOUT` 20s × 4 rounds can exceed client budget).
...
`availability/service.rs:120-138`, `exceptional/service.rs:59-64`, `journey_basis/service.rs:47-52` match on `"no direct trains"`, `"not_found"`, `"timeout"`, `"circuit open"` substrings. Add typed error kinds on `AppError` instead.

### 4.11 Static/local fallbacks answer 200-with-empty instead of honest degradation
`availability/service.rs:182-192`, `exceptional/service.rs:65-73`, `journey_basis/service.rs:129-139`, `live_station/service.rs:112-123,319-370` (a hardcoded 7-train HYB board with static times + `ST####/Static` filler), `pnr/service.rs:167-181` (a `local-validator` candidate that can only ever fail, inflating N). Return honest `503`/`204` + notice, or compute from the dataset.
...
- **`metrics_mw`/obs:** `use_errors_total` is a gauge named like a counter (`obs.rs:344-347`); redundant aliases in `sre.rs` fine-print; `HEDGING_NOTE` const exists only to echo a string.
- **Pinned/known-CVE libs:** verify `h2 0.4.15` and others with `cargo audit`.
...
- **AI:** raw chain-of-thought streamed & rendered (`Assistant.svelte:496-501`); `PERSONA` doesn't forbid code fences (known formatting complaint); `chat_complete` single-shot path built but unused; inbound roles restricted to user/assistant (good).
...
- **H3 — `PLAN.md`** (605 lines) describes a vanilla-JS rearchitecture superseded by Svelte; all `[ ]` checkboxes unchecked. `docs/REARCHITECTURE.md` similarly stale. Either banner as historical or rewrite.
...
- **M4 — `docs/SRE.md`, `src/core/resilience.rs`, `src/core/sre.rs` are untracked** (uncommitted).
- **L3 — `/readyz`, `/rail-api/capacity`** missing from README endpoint table.
...
- **Hermetic tests:** ~167 integration tests across 26 files + ~36 in-src `#[cfg(test)]` modules, all mock-driven; `tests/live.rs` correctly env-gated.
...
- **Security headers** baseline is strong (CSP, `nosniff`, `X-Frame-Options: DENY`, Referrer-Policy).
- **Real metrics** with bounded in-memory path cap, structured JSON logs with rotation, `/healthz` + Render `PORT` fallback.
