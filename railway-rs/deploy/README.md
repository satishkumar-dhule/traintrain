...
needs no AI API keys and uses only public, free data sources. The optional AI assistant (/rail-api/ai/*) relays chat to the OpenCode Zen gateway, keyless on free models by default; set RAILWAY_AI_MODEL / RAILWAY_AI_API_KEY to change it and RAILWAY_AI_ENABLED=false to disable.
