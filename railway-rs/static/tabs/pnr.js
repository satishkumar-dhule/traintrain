/* pnr.js - PNR Status tab. Live enquiry against GET /rail-api/pnr.
   Captcha 428 flow is retried up to 3 total attempts. */
...
window.Tabs.pnr = {
  ...
  mount(root, ctx) {
    ...
    const form = ui.card('',
      ui.label('PNR (10 digits)'),
      input,
      ui.el('div', { class: 'row mt-8' }, btn),
    );
  ...
  const passengers = (Array.isArray(data.passengers) && data.passengers.length)
    ? ui.card('Passengers', ui.table(
        ...
        data.passengers.map((p) => [
          esc(p && p.booking_status),
          esc(p && p.current_status),
          esc(p && p.coach),
          esc(p && p.berth),
        ]),
      ))
